...
| Featured image             | Same key (`gemini-2.5-flash-image`, `imagen-4.0-fast-generate-001`, etc.)         |
...
3. In AI Studio (playground) when tuning prompts:
   - **Model:** `gemini-3.8-flash` (default + primary fallback)
...
4. Free-tier tips:
   - Default is **Gemini 3.8 Flash**; on `503`/`429` SyncApp retries then falls back to Flash Lite
   - Pro often returns `429` and falls back to **3.8 Flash**
   - Image generation uses Gemini multimodal models (`gemini-2.5-flash-image`) and Imagen (`imagen-4.0-fast-generate-001`). Note that free-tier API keys may have 0 quota for image generation until billing is enabled in Google AI Studio.
...
| `GOOGLE_AI_MODEL` | Recommended | `gemini-3.8-flash`               |
...
echo gemini-3.8-flash | npx vercel env add GOOGLE_AI_MODEL production preview --force --yes --no-sensitive
```
